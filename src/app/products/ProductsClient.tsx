...
function ProductsPageContent({ initialProducts = [] }: { initialProducts?: Product[] }) {
  ...
  useEffect(() => {
    ...
    // Skip the first fetch if we already have initial products and no active filters
    if (isInitialMount.current) {
      isInitialMount.current = false;
      if (initialProducts.length > 0 && !searchVal && !newArrivalsVal && !catVal) {
        setLoading(false);
        return;
      }
    }
                    ...
                    <div className="p-4 flex-1 flex flex-col relative">
                      ...
                      <div className="mt-4 pt-4 border-t border-surface-container-low flex items-center justify-between gap-2">
                        <div className="flex-1">
                           <ProductRatingBadge productId={product.id} initialStats={(product as any).reviewStats} />
                        </div>
