...
import { getMerchantOrders, getAllOrders } from "@/lib/api/order";
import OnboardingPopup from "@/components/admin/OnboardingPopup";
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const ordersList = role === 'admin' ? await getAllOrders() : await getMerchantOrders();

        let totalRevenue = 0;
        ordersList.forEach((oData) => {
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-primary-container text-on-surface p-6 border-4 border-on-surface shadow-[4px_4px_0px_0px_var(--color-on-surface)] flex flex-col justify-between hover:-translate-y-0.5 transition-all">
          <div className="flex justify-between items-start">
            ...
            <Link href="/admin/products" className="text-on-surface hover:text-on-surface/80">
        ...
        <div className="bg-secondary-container text-on-surface p-6 border-4 border-on-surface shadow-[4px_4px_0px_0px_var(--color-on-surface)] flex flex-col justify-between hover:-translate-y-0.5 transition-all">
          <div className="flex justify-between items-start">
            ...
            <Link href="/admin/orders" className="text-on-surface hover:text-on-surface/80">
              <ArrowUpRight size={20} />
            </Link>
          </div>
          <div className="mt-6">
